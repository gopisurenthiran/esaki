...
<?php 
  ...
  if(isset($_POST['s1'])){
    if($_POST['s1']=="Save"){
      ...
      $insert = mysqli_query($link,"insert into faculty(autoid,title,sort,status,description)values('$autoid','$title','$sort','$status','$description')");
      
      if($insert){
        header("location:faculty_add.php?msg=1");
      }
    ...
    if($_POST['s1']=="Update"){
      ...
      $update = mysqli_query($link,"update faculty set description='$description',sort='$sort',status='$status',title='$title' where autoid='$autoid'");
     
      if($update){
        header("location:faculty_add.php?pid=$id&msg=2");
      }
    }
  }
            ...
            <div class="box">
              ...
              <div class="box-body">
                ...
                if(isset($_REQUEST['pid'])){
                    $id = $_REQUEST['pid'];
                  $sel_rw = mysqli_query($link,"select * from faculty where id=$id"); 
                  ...
                  $autoid=$row->autoid; 
                    ...
                    list($max) = mysqli_fetch_row(mysqli_query($link,"SELECT MAX(id) AS max FROM `faculty`"));
