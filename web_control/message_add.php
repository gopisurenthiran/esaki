...
<?php 
  ...
  if(isset($_FILES['upload_picture'])) 
  {
    ...
    if(count($err) === 0) {
      ...
      $insert = mysqli_query($link,"insert into messages(autoid,description,details,sort,status,title)values('$autoid','$description','$details','$sort','$status','$title')");
      ...
      if($upload_picture != "")
        {
          ...
          mysqli_query($link,"UPDATE `messages` SET `imgurl` = '$upload_pic' WHERE autoid ='$autoid'");
        }

      header("location:message_add.php?msg=1");
    }
  }
}
    if($_POST['s1']=="Update"){
     ...
     $description = $_POST['description'];
      ...
      $update = mysqli_query($link,"update messages set description='$description',details='$details',sort='$sort',status='$status',title='$title' where autoid='$autoid'");
  ...
  if(isset($_FILES['upload_picture'])) 
  {
    ...
    if(count($err) === 0) {
      ...
      mysqli_query($link,"UPDATE `messages` SET `imgurl` = '$upload_pic' where autoid='$autoid'");
        ...
        header("location:message_add.php?pid=$id&msg=2");
  }else{
        header("location:message_add.php?pid=$id&msg=2");
      }
            ...
            <div class="box">
              ...
              <div class="box-body">
                ...
                if(isset($_REQUEST['pid'])){
                    $id = $_REQUEST['pid'];
                  $sel_rw = mysqli_query($link,"select * from messages where id=$id"); 
                  ...
                  $autoid=$row->autoid; 
                    ...
                    list($max) = mysqli_fetch_row(mysqli_query($link,"SELECT MAX(id) AS max FROM `messages`"));
               ...
               <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 no-pad">
                    <div class="form-group">
                      <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 no-pad">
                        <label class="margin-left-50" for="childcategory_id">Tab Name</label>
                      </div>
                      <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-pad">
                  ...
                  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-pad">
                    <div class="form-group">
                      <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 no-pad">
                        <label class="margin-left-50" for="childcategory_id">Details</label>
                      </div>
                      <div class="col-lg-10 col-md-10 col-sm-12 col-xs-12 no-pad">
                        <textarea class="form-control ckeditor" name="details"  required><?php echo $row->details; ?></textarea>
                      </div>
                    </div> 
                  </div>
                  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 no-pad">
